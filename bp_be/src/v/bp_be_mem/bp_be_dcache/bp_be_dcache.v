...
module bp_be_dcache
 ...
 #(parameter bp_params_e bp_params_p = e_bp_inv_cfg
  ...
  assign addr_bank_offset_tv = paddr_tv_r[byte_offset_width_lp+:bank_offset_width_lp];
  ...
  bsg_mux #(
    ...
    .data_i(ld_data_tv_r)
    ,.sel_i(load_hit_way_tv_r ^ addr_bank_offset_tv)
  ...
  logic [dcache_assoc_p-1:0]                                  data_mem_write_bank_mask, data_mem_write_mask;
  ...
  for (genvar i = 0; i < dcache_assoc_p; i++) begin
    ...
    assign data_mem_mask_li[i] = wbuf_yumi_li
      ? wbuf_mask
      : {data_mem_mask_width_lp{data_mem_write_mask[i]}}; // use fill_index to generate write_mask
