# TODO [STD]: This file should be merged with bsg_chip_timing_constraint.tcl
#             to handle both cases were the input clocks are generated clocks
#             or standard clocks.
...
proc bsg_chip_ucsd_bsg_332_timing_generated_clocks_constraint {bsg_reset_port \
                                              bsg_core_clk_port \
                                              bsg_core_clk_name \
                                              bsg_core_clk_period \
  ...
  puts "INFO\[BSG\]: begin bsg_chip_ucsd_bsg_332_timing_generated_clocks_constraint function in script [info script]\n"

  set core_clk_obj [get_generated_clocks $bsg_core_clk_name]
  set master_io_clk_obj [get_generated_clocks $bsg_master_io_clk_name]
  ...
  set_clock_groups -physically_exclusive \
                   -group [append_to_collection [get_generated_clocks *] [remove_from_collection [get_clocks *] [get_clocks *_BSG_CHIP_cdc]]] \
  ...
  set_clock_uncertainty $core_clk_uncertainty $core_clk_obj
  set_clock_uncertainty $master_io_clk_uncertainty $master_io_clk_obj
  ...
  puts "INFO\[BSG\]: end bsg_chip_ucsd_bsg_332_timing_generated_clocks_constraint function in script [info script]\n"

}

proc bsg_chip_timing_generated_clocks_constraint {args} {
  ...
  ##########################################################
  # Optional parameters for manycore clock
  if {[info exist pargs(-manycore_clk_port) ] } {
        set bsg_manycore_clk_port $pargs(-manycore_clk_port)
  } else {
        set bsg_manycore_clk_port ""
  }

  if {[info exist pargs(-manycore_clk_name) ] } {
        set bsg_manycore_clk_name $pargs(-manycore_clk_name)
  } else {
        set bsg_manycore_clk_name ""
  }

  if {[info exist pargs(-manycore_clk_period) ] } {
        set bsg_manycore_clk_period $pargs(-manycore_clk_period)
  } else {
        set bsg_manycore_clk_period ""
  }

  if {[info exist pargs(-create_manycore_clk) ] } {
        set bsg_create_manycore_clk $pargs(-create_manycore_clk)
  } else {
        set bsg_create_manycore_clk 0
  }
  ...
  if {$bsg_package != "ucsd_bsg_332" || $bsg_core_clk_period <= 0 || $bsg_master_io_clk_period <=0} {
    ...
    bsg_chip_ucsd_bsg_332_timing_generated_clocks_constraint \
                     $bsg_reset_port \
                     $bsg_core_clk_port \
                     $bsg_core_clk_name \
                     $bsg_core_clk_period \
...
define_proc_attributes bsg_chip_timing_generated_clocks_constraint \
  ...
  -define_args {
      ...
      {-manycore_clk_port "manycore clock port, i.e \[get_ports p_misc_L_i\[5\]\]" manycore_clk_port string optional}
      {-manycore_clk_name "manycore clock name, i.e core_clk" manycore_clk_name string optional}
      {-manycore_clk_period "core clock period in tech-time-unit, i.e 3.5" manycore_clk_period float optional}
      ...
      {-create_manycore_clk "0 if don't create" create_manycore_clk string optional}
