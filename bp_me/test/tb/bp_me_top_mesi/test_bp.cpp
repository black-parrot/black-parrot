...
int sc_main(int argc, char **argv) 
{
  ...
  sc_signal <sc_bv<bp_lce_lce_tr_resp_width> > lce_tr_resp_i("lce_tr_resp_i");
  sc_signal <bool>     lce_tr_resp_v_i("lce_tr_resp_v_i");
  sc_signal <bool>     lce_tr_resp_ready_o("lce_tr_resp_ready_o");

  sc_signal <sc_bv<bp_lce_lce_tr_resp_width> > lce_tr_resp_o("lce_tr_resp_o");
  ...
  DUT.lce_tr_resp_i(lce_tr_resp_i);
  DUT.lce_tr_resp_v_i(lce_tr_resp_v_i);
  DUT.lce_tr_resp_ready_o(lce_tr_resp_ready_o);
  ...
  for (int i = 0; i < TRACE_ITERS; i++) {
    ...
    uint32_t nonExclReq = rand() % 1;
    lce_req_i = createLceReq(0, 0, reqType, reqAddr, (bp_lce_cce_req_non_excl_e)nonExclReq, lruWay, e_lce_req_lru_clean).to_uint();
    lce_req_v_i = 1;
    sc_start(CLK_TIME, SC_NS);
    lce_req_i = 0;
    lce_req_v_i = 0;
