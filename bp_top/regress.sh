...
for ROM in $ISA_ROMS ; do
  echo -n "$ROM : "
  make -C $BP_TOP_DIR/syn TEST_ROM=$ROM.v TRACE_ROM=$ROM.tr.v COVERAGE=VCS  bp_single_trace_demo.run.v \
    | grep "PASS" || echo "FAIL"
done
...
for ROM in $BENCH_ROMS ; do
  echo -n "$ROM : "
  make -C $BP_TOP_DIR/syn TEST_ROM=$ROM.v TRACE_ROM=$ROM.tr.v COVERAGE=VCS bp_s\
ingle_trace_demo.run.v \
    | grep "PASS" || echo "FAIL"
done
