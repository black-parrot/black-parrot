...
, parameter int num_int_repeater_nodes_p [num_router_groups_p-1:0] = '{default: 0}
)
...
, output logic [num_router_groups_p-1:0][wh_cord_width_lp-1:0] rtr_cord_o
  ...
  bsg_chip_io_complex_router_network #(.ct_width_p( ct_width_p )
                                      ...
                                      ,.num_int_repeater_nodes_p ( num_int_repeater_nodes_p )
                                      )
    network
      (.core_clk_i( core_clk_i )
...
, parameter     wh_cord_width_lp                = wh_cord_markers_pos_p[1] - wh_cord_markers_pos_p[0]
, parameter     tag_cord_width_p                = wh_cord_width_lp
, parameter     num_edge_repeater_nodes_p       = 0
, parameter int num_int_repeater_nodes_p [num_router_groups_p-1:0] = '{0}
, parameter     bsg_ready_and_link_sif_width_lp = `bsg_ready_and_link_sif_width(ct_width_p)
)
...
, output [num_in_p-1:0][bsg_ready_and_link_sif_width_lp-1:0] east_links_o
  ...
  bsg_noc_repeater_node #(.width_p    ( ct_width_p )
                         ,.num_in_p   ( num_in_p )
                         ,.num_nodes_p( num_edge_repeater_nodes_p )
                         )
    west_nodes
      (.clk_i         ( core_clk_i )
      ...
      for (i = 0; i < num_in_p; i++)
        begin: ct
          ...
          if (j > 0)
            begin
              bsg_noc_repeater_node #(.width_p(ct_width_p)
                                      ,.num_nodes_p(num_int_repeater_nodes_p[j-1])
                                      )
                int_repeater
                  (.clk_i           ( core_clk_i )
                  ,.side_A_reset_i  ( rtr_core_tag_data_lo[j-1].reset )
                  ,.side_A_links_i  ( rtr_links_lo[j-1][i][E] )
                  ,.side_A_links_o  ( rtr_links_li[j-1][i][E] )

                  ,.side_B_links_i  ( rtr_links_lo[j][i][W] )
                  ,.side_B_links_o  ( rtr_links_li[j][i][W] )
                  );
            end
          // East end link
